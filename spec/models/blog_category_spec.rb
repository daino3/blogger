...
describe BlogCategory do
  describe '#with_blogs' do
    let(:cat_with_blog) { FactoryGirl.create(:blog_category, :with_blogs) }
    ...
    it 'limits the blog categories that actually have published blog posts' do
      ...
      expect(BlogCategory.with_posts.count).to eq(1)
      expect(BlogCategory.with_posts.pluck(:name)).to include(cat_with_blog.name)
    end
  end
